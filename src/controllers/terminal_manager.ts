import { getWebSocketBaseURL, serverRequest } from '@/utils/request';
...
class TerminalManager {
  ...
  connectTerminal (
    ...
    if (!conn) {
      ...
      // WebSocket 端点不需要 /api 前缀，需要移除
      const wsUrlWithoutApi = wsBaseUrl.replace(/\/api$/, '');
      const url = new URL(`${wsUrlWithoutApi}/ws/terminal`, window.location.href);
