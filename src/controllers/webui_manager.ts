...
import { getBaseURL, serverRequest } from '@/utils/request';
...
export default class WebUIManager {
  ...
  public static getRealTimeLogs (writer: (data: Log[]) => void) {
    ...
    const eventSource = new EventSourcePolyfill(`${baseUrl}/Log/GetLogRealTime`, {
      headers: {
  ...
  public static getSystemStatus (writer: (data: SystemStatus) => void) {
    ...
    const eventSource = new EventSourcePolyfill(
      `${baseUrl}/base/GetSysStatusRealTime`,
      {
        headers: {
          Authorization: `Bearer ${_token}`,
          Accept: 'text/event-stream',
