...
import { getWebSocketBaseURL } from '@/utils/request';

// 生成默认WebSocket URL，基于用户配置的后端地址
const getDefaultWsUrl = (): string => {
  const wsBaseUrl = getWebSocketBaseURL();
  if (wsBaseUrl) {
    // 移除 /api 后缀，改为 3001 端口
    const url = new URL(wsBaseUrl.replace(/\/api$/, ''));
    url.port = '3001';
    return url.href;
  }
  return '';
...
export default function WSDebug () {
  ...
  // 当配置的后端地址变化时，自动更新默认URL
  useEffect(() => {
    const defaultUrl = getDefaultWsUrl();
    if (defaultUrl && !socketConfig.url) {
      setSocketConfig({ ...socketConfig, url: defaultUrl });
