...
// 生成默认HTTP URL，基于用户配置的后端地址
const getDefaultHttpUrl = (): string => {
  const baseUrl = getBaseURL();
  if (baseUrl) {
    // 移除 /api 后缀，改为 3000 端口
...
const OneBotApiDebug: React.FC<OneBotApiDebugProps> = (props) => {
  ...
  useEffect(() => {
    const defaultUrl = getDefaultHttpUrl();
    if (defaultUrl && !httpConfig.url) {
      setHttpConfig({ ...httpConfig, url: defaultUrl });
    }
  }, []);
